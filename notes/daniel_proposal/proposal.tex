...
\begin{itemize}
    ...
    \]
    where $\Delta u \equiv u(w) - u(c_f) > 0$. Following the terminology of \cite{allcott2019regressive}, this is an \emph{internality correction}: it captures the welfare gain from switching the marginal household---which misperceives its own risk---to insurance. It is positive when $q^* < p$, i.e.\ when the marginal household underestimates flood risk.
    ...
    \end{align}
    At an interior optimum, $\text{MVPF}_s = \text{MVPF}_a = \lambda$. At a corner (e.g.\ $a=0$), the unused instrument has a lower MVPF. The \emph{fiscal externality} in the denominator arises because behavioral responses to policy change the government's costs: when a household switches from relief to insurance (or vice versa), per-capita spending changes by $(s-a)pd$.
...
\begin{itemize}
    ...
    \item The left-hand side is the \emph{internality correction}, amplified by the behavioral responsiveness of insurance take-up to each instrument (per recipient). The term $\frac{1}{I_0}\frac{\partial I}{\partial s}\big|_0$ measures how effectively subsidies attract marginal households into insurance per insured household, and $\frac{1}{U_0}|\frac{\partial I}{\partial a}\big|_0|$ measures how many households crowd out of insurance per uninsured household when relief increases.
    \item The right-hand side reflects the \emph{consumption-smoothing advantage of disaster relief}: since $c_f^0 < c_{ins}^0$, marginal utility is higher for uninsured households in a flood ($u'(c_f^0) > u'(c_{ins}^0)$), so a dollar of relief generates more utility than a dollar of subsidy on the transfer margin. 
    \item In words: subsidies should receive more funding when the welfare gain from correcting underinsurance (weighted by how responsive take-up is) exceeds the consumption-smoothing benefit of directing transfers to the neediest households.
\end{itemize}
