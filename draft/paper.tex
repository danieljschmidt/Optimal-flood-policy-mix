...
Homeowners with GSE-backed mortgages form a special category of aid recipients. A disaster declaration that releases Individual Assistance funding also makes homeowners eligible for forbearance for up to 12 months on their mortgage repayments for GSE-backed loans.\footnote{Information accessed via this \hyperlink{https://www.fhfa.gov/homeowners-and-homebuyers/mortgage-assistance/disaster-assistance}{link}.} Furthermore, it has been argued that mortgage lenders' ability to sell securitised mortgages to GSEs encourages them not to adjust loan conditions to flood-related default risk, giving homeowners access to underpriced implicit insurance against flood damages  \citep{keenan2020underwaterwriting,sastry2023insurers}. \cite{bakkensen2025leveraging} and \cite{liaomulder2026homeequity} find evidence that homeowners exploit this insurance option in a strategic manner.\footnote{\cite{bakkensen2025leveraging} show that buyers identified as pessimistic regarding climate change are more likely to use a mortgage when purchasing properties exposed to sea level rise and have higher loan-to-value ratios,. \cite{liaomulder2026homeequity} show that owners of highly leveraged properties are less likely to purchase flood risk insurance.} However, \cite{gallagher2017household} note that Hurricane Katrina engendered at most a modest rise in defaults, while \cite{sastry2026bears} demonstrates that mortgage lenders restrict credit in flood-prone areas to avoid excess losses, mainly by reducing maximum loan-to-value ratios.\footnote{\cite{nguyen2022climate} provide earlier evidence of reduced LTVs in flood-prone areas, but, using data , note that providers underestimate.} \citep{qin2026flood}. 


\subsection{Adaptation}

Notes on subsidies and regulations encouraging regulation, and on public flood protection programmes (covering 1.5$\%$ of homeowners, \citealp{qin2026flood}).

\section{The model}
\label{sec:model}

We formalise the trade-off described above in a one-period model of a single flood-prone region. A social planner allocates a fixed budget $B$ between two policy instruments: a proportional subsidy $s$ to the actuarially fair flood insurance premium, and a fraction $a$ of uninsured flood damages compensated through disaster aid. Households differ only in their subjective beliefs about flood risk and choose whether to buy insurance at the subsidised premium.

\subsection{Environment}

A continuum of households each hold wealth $w$ and face a flood that occurs with true, common probability $p$ and causes damage $d$, a fixed proportion of wealth. Households are correct about the size of the damage $d$ should a flood occur, but hold heterogeneous subjective beliefs $q$ about the probability that it occurs, distributed across the population according to the CDF $F(q)$. If a household buys insurance, the government subsidises its premium so that it pays $(1-s)pd$ rather than the actuarially fair premium $pd$; note that the premium reflects the true flood probability $p$ even though take-up decisions are governed by subjective beliefs. If a household remains uninsured and a flood occurs, disaster aid pays the household a fraction $a$ of its damage.

\subsection{Household insurance decisions}

An insured household's consumption is unaffected by whether a flood occurs, since the insurance premium is paid regardless and the payout exactly offsets the loss. The utility of an insured household is therefore
\begin{align}
    V_I(s) = u(c_I), \quad \text{with} \quad c_I = w - (1-s)pd.
\end{align}
An uninsured household consumes $c_{U,N} = w$ if no flood occurs and $c_{U,F} = w-(1-a)d$ if one does. Evaluated at the true flood probability $p$, as the social planner does, its expected utility is
\begin{align}
    V_U(a) &= (1-p)\,u(c_{U,N}) + p\,u(c_{U,F}) = u(c_{U,N}) - p\cdot\Delta u,
\end{align}
where $\Delta u \equiv u(c_{U,N})-u(c_{U,F})>0$ is the utility loss an uninsured household suffers from a flood. A household with subjective belief $q$, however, perceives its own expected utility from remaining uninsured as
\begin{equation}
    \tilde{V}_U(q,a) = u(c_{U,N}) - q\cdot\Delta u.
\end{equation}
This household buys insurance whenever $V_I(s) > \tilde{V}_U(q,a)$, that is, whenever its subjective belief exceeds a threshold $q^*(s,a)$ implicitly defined by indifference between the two options:
\begin{equation}
    q^*\cdot\Delta u = u(c_{U,N}) - u(c_I). \label{eq:qstar_prog}
\end{equation}
The threshold $q^*$ is the belief at which the perceived utility cost of facing flood risk uninsured just equals the utility cost of paying the subsidised premium. Households with $q>q^*(s,a)$ buy insurance and households with $q\le q^*(s,a)$ do not, so that the insurance take-up rate is $I(s,a) = 1-F(q^*(s,a))$. Because $q^*$ does not depend on the belief distribution itself, this threshold condition holds regardless of how beliefs are distributed across the population.

\subsection{The social planner's problem}

The planner evaluates household welfare using the true flood probability $p$, not households' subjective beliefs, so a household's own insurance decision is not necessarily the one that a fully-informed planner would have chosen. Social welfare is the population average of household utility,
\begin{equation}
    S(s,a) = V_I(s)\cdot I(s,a) + V_U(a)\cdot\bigl(1-I(s,a)\bigr), \label{eq:welfare_prog}
\end{equation}
maximised subject to the budget constraint that total spending on subsidies to insured households and aid to uninsured flood victims not exceed $B$:
\begin{equation}
    B = pd\bigl[s\,I(s,a) + a\,\bigl(1-I(s,a)\bigr)\bigr]. \label{eq:budget_prog}
\end{equation}
...
\end{equation}
At an interior optimum $\text{MVPF}_s = \text{MVPF}_a = \lambda$, so that neither instrument can be improved upon at the margin; away from the optimum, the planner should shift the marginal dollar of the budget towards whichever instrument has the higher MVPF.

Each numerator combines two channels through which the corresponding instrument raises welfare. The first, a direct benefit, is the value of the transfer to households who are already making the same choice: the subsidy benefits every insured household by $\partial V_I/\partial s = pd\,u'(c_I)$ per unit, while aid benefits every uninsured flood victim by $\partial V_U/\partial a = pd\,u'(c_{U,F})$ per unit. Because uninsured flood victims have lower consumption than insured households, they also have higher marginal utility, so a dollar of aid is worth more to its recipients than a dollar of subsidy is worth to insured households, holding take-up fixed. The second channel operates through households whom the policy change induces to switch between insurance and no insurance. Since the marginal household is, by construction, indifferent between the two options according to its own subjective belief, but not according to the planner's evaluation at the true probability $p$, each switcher generates a behavioural internality of
\begin{equation}
    V_I - V_U = \tilde{V}_U(q^*,a) - V_U(a) = (p-q^*)\,\Delta u \label{eq:internality_prog}
\end{equation}
per household \citep[following the internality terminology of][]{allcott2019regressive}. The sign of this term depends on where the marginal household's belief $q^*$ sits relative to the true probability $p$: it is positive whenever $q^*<p$, i.e.\ whenever the household at the margin underestimates its true flood risk. This is not a property guaranteed by the model's structure for arbitrary $(s,a)$, but it is what we find to hold at current policy and throughout our calibrated range, consistent with the survey and market evidence on underestimation discussed in Section~\ref{sec:motivation}. Where it holds, being pushed into insurance by a more generous subsidy makes the switching households better off by the planner's own reckoning, while being pulled out of insurance by more generous aid makes them worse off.

Each denominator likewise combines two components. First, a direct fiscal cost, $pd$ per recipient, applied to the $I$ insured households for the subsidy or the $1-I$ uninsured households for aid and second a fiscal spillover (or fiscal externality). A household that switches from being uninsured to insured (or vice versa) changes the government's fiscal exposure to that household by $(s-a)pd$, since it now costs $spd$ rather than $apd$ to support, or vice versa. This creates a spillover between the two instruments' budgets: part of the fiscal cost of expanding one policy is offset by reduced spending on the other, because both a more generous subsidy and more generous aid change the same take-up margin $I(s,a)$, just in opposite directions.

\subsection{Take-up responses and the MVPF in terms of deep parameters}
