...
Annual flood probability & $p$ & 0.02 & NFIP claims / G\&S (2026) \\
\midrule
Insurance subsidy & $s$ & 0.47 & GAO (2023) \\
Disaster relief fraction & $a$ & 0.14 & Gruber \& Solomon (2026) \\
\midrule
Insurance take-up rate & $I$ & 0.30 & FEMA OpenFEMA \\
Semi-elasticity of take-up ($\ln$-price) & $\varepsilon$ & -0.32 & Gruber \& Solomon (2026) \\
\bottomrule
\end{tabular}
\end{table}
...
We set the annual flood probability to $p=0.02$, corresponding to an average annual flood probability of 2\% among SFHA properties. The SFHA threshold is 1\% (100-year floodplain), but the average probability across properties inside the SFHA is higher. This value is consistent with the fiscal-spillover cross-check: the model implies $a \times p \times d \approx \$210$ per house per percentage-point increase in coverage, in line with Gruber \& Solomon's (2026) IV estimate of \$175--\$203.
...
Let $\pi=(1-s)pd$ denote the insurance premium. Define the semi-elasticity of take-up with respect to the log-premium as
\[
    \varepsilon \equiv \frac{\partial I}{\partial\ln\pi} < 0.
\]
...
\]
Since $\partial I/\partial\pi = \varepsilon/\pi$, solving for $f(q^*)$:
\begin{equation}
    f(q^*) = -\frac{\varepsilon}{\pi}\,\frac{\Delta u}{u'(c_I)}. \label{eq:fqstar}
\end{equation}
The semi-elasticity $\varepsilon$ is the single empirical input: it can be estimated from variation in insurance premiums across households or over time. Given $\varepsilon$ and $I$, all remaining objects on the right-hand side---$\pi$, $\Delta u$, and $u'(c_I)$---are computable from the structural parameters $(w,d,s,a,\gamma)$.

\paragraph{Global evaluation: Fitting the belief distribution.}
For global evaluations---tracing the optimal policy frontier or computing welfare at counterfactual policies far from the status quo---the complete distribution $F(q)$ is required. We fit a Beta distribution $q\sim\mathrm{Beta}(\alpha,\beta)$, whose two free parameters are identified by two moment conditions. Let $\hat{f} \equiv -(\varepsilon/\pi)(\Delta u/u'(c_I))$ denote the target density derived in~\eqref{eq:fqstar}. The conditions are:
\begin{align}
    I_{q^*}(\alpha,\beta) &= 1 - I, \label{eq:beta_cdf} \\
    \frac{(q^*)^{\alpha-1}(1-q^*)^{\beta-1}}{B(\alpha,\beta)} &= \hat{f}, \label{eq:beta_pdf}
\end{align}
where $I_{q^*}(\alpha,\beta)$ is the regularized incomplete beta function and $B(\alpha,\beta)=\Gamma(\alpha)\Gamma(\beta)/\Gamma(\alpha+\beta)$. Condition~\eqref{eq:beta_cdf} requires the fraction of the population with beliefs at or below $q^*$ to equal the observed uninsured share $1-I$; condition~\eqref{eq:beta_pdf} requires the density at $q^*$ to equal the value pinned down by the premium semi-elasticity. Given the structural objects $(q^*,\pi,\Delta u,u'(c_I))$---all computable from $(w,d,s,a,\gamma)$---together with the empirical inputs $(I,\varepsilon)$, the system~\eqref{eq:beta_cdf}--\eqref{eq:beta_pdf} uniquely identifies $(\alpha,\beta)$ and is solved numerically.
