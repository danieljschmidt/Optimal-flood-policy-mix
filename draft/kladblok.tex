...
Note that, so far, we have not made any assumptions about the difficult-to-observe distribution $F(q)$ of flood risk belief across households. The terms in \eqref{eq:mvpf_prog} are current policy $(s,a)$, a process for flooding, assumptions regarding household wealth and risk aversion, and finally empirically observable insurance demand. We are thus able to calibrate each term and obtain the marginal value of each policy instrument while remaining agnostic about the distribution of household beliefs (although clearly not about the belief $q^*$ of the marginal household). 

\subsubsection{Optimal policy $(s^*,a^*)$}
Away from current policy $(s,a)$, the empirical values of the behavioural terms in \eqref{eq:mvpf_prog} obtained from recent data (namely, the insurance take-up $I$ and its derivatives w.r.t. the policy instruments) are no longer valid. Therefore, to find the optimal combination of policy instruments $(s^*,a^*)$, we must posit a distribution of flood risk beliefs $F(q)$. This distribution then implies the insurance take-up rate $I=1-F(q^*(s^*,a^*))$ and its marginal response to policy.
...
\begin{align}
    \text{MVPF}_s &= \frac{u'(c_I)\bigl[I + (p-q^*)\,f(q^*)\bigr]}{I + (s-a)\,\dfrac{p\overline{d}\,u'(c_I)\,f(q^*)}{\Delta u}}, \notag \\
    \text{MVPF}_a &= \frac{\dfrac{\mathbb{E}[d\,u'(c_{U,F}(d))]}{\overline{d}} \! \left[(1-I) - \dfrac{(p-q^*)q^*f(q^*)}{p}\right]}{(1-I) - (s-a)\,\dfrac{q^*d\,\mathbb{E}[d\,u'(c_{U,F}(d))]\,f(q^*)}{\Delta u}}. \label{eq:mvpf_deep_prog}
\end{align}

These terms summarise the arguments rehearsed above. Recall that the numerator of each term describes the utility effect of the policy and the denominator its cost. Net of any behavioural effect, disaster aid has a stronger effect than the insurance subsidy on utility (in the numerator, $\mathbb{E}[d\,u'(c_{U,F}(d))]/\overline{d}>u'(c_I)$). Meanwhile, the behavioural effect of the subsidy on utility is positive and that of disaster aid is negative (in the numerator, see the terms containing the belief wedge $(p-q^*)$). However, the behavioural response to disaster aid is dampened given that $q^*<p$ (both in the numerator and denominator of the second term). Finally, in the denominator, the fiscal spillover is represented by the difference $(s-a)$ as before. 

As mentioned, the terms in \eqref{eq:mvpf_deep_prog} can only be quantified given a belief distribution $F(q)$. Although we cannot observe this distribution directly, clearly a minimum requirement of a candidate distribution is that it ensures \eqref{eq:mvpf_prog} and \eqref{eq:mvpf_deep_prog} are consistent: given current policy $(s,a)$, the insurance take-up $I=1-F(q^*(s,a))$ should match its observed level, and the density $f(q^*(s,a))$ should reproduce the marginal response of insurance demand to policy which we insert directly into \eqref{eq:mvpf_prog} using empirical estimates. We describe our calibration strategy for both equations in greater detail below. 
